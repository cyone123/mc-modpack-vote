...
export const storage = {
  ...
  async vote({ packId, playerId, playerName, ip, deviceId, fingerprint }) {
    ...
    let action = 'added';
    if (existingIndex >= 0) {
      const existingVote = pack.votes[existingIndex];

      // Check if the current client is authorized to retract this vote.
      // Must match either deviceId or fingerprint (or IP for legacy records).
      const hasSecurityData = Boolean(
        existingVote.deviceId || existingVote.fingerprint || existingVote.ip
      );

      if (hasSecurityData) {
        let isAuthorized = false;

        // 1. Persistent device ID match
        if (deviceId && existingVote.deviceId && deviceId === existingVote.deviceId) {
          isAuthorized = true;
        }
        // 2. Hardware / canvas fingerprint match (covers Incognito / cache cleared)
        else if (fingerprint && existingVote.fingerprint && fingerprint === existingVote.fingerprint) {
          isAuthorized = true;
        }
        // 3. IP match fallback (only when deviceId and fingerprint were not recorded originally)
        else if (
          !existingVote.deviceId &&
        ...
        if (!isAuthorized) {
          throw new Error(`无法取消投票：该选票由其他设备投出，当前设备与原投票记录不匹配`);
        }
      }
  ...
  async deleteUserPack({ packId, playerName, ip, deviceId, fingerprint }) {
    await ensureLoaded();
    ...
    // Check device / fingerprint of creator (stored in creator's initial vote)
    const creatorVote = pack.votes?.[0];
    if (creatorVote && (creatorVote.deviceId || creatorVote.fingerprint)) {
      let isCreator = false;
      if (deviceId && creatorVote.deviceId && deviceId === creatorVote.deviceId) {
        isCreator = true;
      } else if (fingerprint && creatorVote.fingerprint && fingerprint === creatorVote.fingerprint) {
        isCreator = true;
      }
      if (!isCreator) {
        throw new Error("无法删除该整合包：该整合包由其他设备自荐，当前设备与自荐人设备不匹配");
      }
    }
